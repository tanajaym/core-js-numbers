...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const vectorLength1 = Math.sqrt(Math.pow(x1, 2)+Math.pow(y1, 2));
  const vectorLength2 = Math.sqrt(Math.pow(x2, 2)+Math.pow(y2, 2));

  const angle = Math.acos(dotProduct/(vectorLength1*vectorLength2) );
  return angle;
}
